 ...
 * 2. import the eazy-portal-core BOM so every eazy-portal-core-* dependency resolves to one aligned version,
 * 3. wire sibling + eazy-portal-core dependencies ([configureDependencies]),
 * 4. add `eazy-portal-core-test` on `testImplementation` (every module gets it, §5.3).
 *
...
internal abstract class GradleProjectConfigurer(
    ...
    /** Wires the archetype's sibling + eazy-portal-core dependencies via [applySiblingProject] / [applyEazyPortalCore]. */
    protected abstract fun configureDependencies()

    /** Adds the versionless `eazy-portal-core-$projectType` dependency on [configuration] and records it. */
    ...
    private fun applyEazyPortalCore(
        ...
        eazyPortalCoreDependencies += WiredDependency(dependencyConfiguration, "eazy-portal-core-$projectName")
    }
    ...
    /** eazy-portal-core notation WITHOUT a version — the BOM (see [importEazyPortalCoreBom]) pins it. */
    private fun createEazyPortalCoreNotation(projectName: String): String =
        "org.eazyportal.gradle:eazy-portal-core-$projectName"

    /** Imports the eazy-portal-core BOM/platform so every eazy-portal-core-* dependency resolves to one aligned version. */
...
//                // api/client expose eazy-portal-core transitively — propagate the BOM constraints to their consumers too.
